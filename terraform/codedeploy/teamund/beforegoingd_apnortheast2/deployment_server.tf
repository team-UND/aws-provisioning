resource "aws_codedeploy_app" "server" {
  name = "codedeploy-app-server-${data.terraform_remote_state.vpc.outputs.shard_id}"
...
resource "aws_codedeploy_deployment_group" "server" {
  ...
  tags = {
    Name        = "codedeploy-group-server-${data.terraform_remote_state.vpc.outputs.shard_id}"
    Environment = data.terraform_remote_state.vpc.outputs.billing_tag
  }
}
