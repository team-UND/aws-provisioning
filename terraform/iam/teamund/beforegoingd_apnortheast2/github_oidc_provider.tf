  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          data.terraform_remote_state.ecr.outputs.aws_ecr_repository_beforegoingd_server_build_arn,
          data.terraform_remote_state.ecr.outputs.aws_ecr_repository_beforegoingd_prometheus_build_arn
        ]
      },
      {
        Effect = "Allow"
